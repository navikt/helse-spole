...
class SykepengeperioderTest {
    ...
    fun `Behold høyre siden, når venstre er tom`() {
        val perioderA = Sykepengeperioder("a", listOf(Periode(LocalDate.now(), LocalDate.now(), "100", "100", Kilde.INFOTRYGD)))
        val perioderB = Sykepengeperioder("a", emptyList())
    ...
    fun `Når begge sider har disjunkte perioder, så skal alle være med`() {
        val template = Periode(fom = LocalDate.now(), tom = LocalDate.now(), sykemeldingsgrad = "100", utbetalingsgrad = "100", kilde = Kilde.INFOTRYGD)
